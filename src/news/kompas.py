...
# Sitemap names to include — all others are skipped
KOMPAS_ALLOWED_SITEMAPS: set[str] = {
    "global", "cekfakta",
...
def _collect_article_sitemaps(root: ET.Element, depth: int = 0) -> list[str]:
    ...
    for sitemap_tag in root.findall(".//sm:sitemap", NS_SITEMAP):
        ...
        sitemap_name = href.split("sitemap-news-")[-1].replace(".xml", "")
        if sitemap_name not in KOMPAS_ALLOWED_SITEMAPS:
            print(f"{indent}[Sitemap] Skipped (not relevant): {sitemap_name}")
            continue
...
if __name__ == "__main__":
    ...
    if result is not None:
        ...
        categories = set(
            u.split("kompas.com")[0].split("//")[-1].replace("www.", "")

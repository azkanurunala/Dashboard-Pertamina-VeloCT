...
async def inspect():
    ...
    logger.info("🔍 Inspecting data_eia_market table...")

    try:
        # Check columns
        query = "SELECT TOP 1 * FROM data_eia_market"
        result = await db_handler.execute_query(query)
        
        if not result:
            logger.info("Table exists but is empty.")
            # Verify columns by checking empty result keys if possible, or just trust it exists
            # Actually execute_query returns list of dicts, so if empty list, we IDK columns.
            # Let's try to get column info from schema
            schema_query = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'data_eia_market'"
        ...
        # Count rows
        count_query = "SELECT COUNT(*) as count FROM data_eia_market"

...
async def ingest_bioetanol():
    print("Starting Bioetanol Ingestion...")
    config_manager.reload()
    db_config = await config_manager.get_database_config()
    db_handler = DatabaseHandler(db_config)
    
    file_path = r'azure_functions\references\data\(Data)Bioetanol.csv'
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    price_str = row.get('HIP Bioetanol IDR/L', '0').replace(',', '')
                    price = float(price_str) if price_str else 0.0
                    
                    # Note: Bioetanol file also has 'Harga Tetes Tebu', but our schema currently targets price_idr_liter.
                    # We can evolve the schema later if needed, but for now we match the basic mapping.
                    data.append({
    ...
    print(f"Saving {len(data)} rows to data_bioetanol_hip...")
    count = await db_handler.save_structured_data('data_bioetanol_hip', data)
    print(f"Successfully ingested {count} rows!")

if __name__ == "__main__":
    asyncio.run(ingest_bioetanol())

...
TEMPO_ALLOWED_SUBDOMAINS: set[str] = {
    "nasional", "bisnis", "ekonomi", "lingkungan",
    "wawancara", "sains", "investigasi", "cekfakta",
    "kolom", "internasional", "politik", "hukum",
}

# Selenium Search

def _is_allowed_tempo_url(href: str) -> bool:
    """Filter URL Tempo berdasarkan subdomain atau kanal di path."""
    try:
        # Hapus protokol
        without_proto = href.split("//")[1]
        subdomain = without_proto.split(".tempo")[0]

        # Artikel dengan subdomain spesifik (bisnis.tempo.co, nasional.tempo.co, dll)
        if subdomain in TEMPO_ALLOWED_SUBDOMAINS:
            return True

        # Artikel di www.tempo.co — cek kanal dari path
        if subdomain == "www":
            path = without_proto.split("tempo.co/")[1]
            kanal = path.split("/")[0]
            return kanal in TEMPO_ALLOWED_SUBDOMAINS
    ...
    try:
        ...
        for page in range(1, max_pages + 1):
            ...
            page_urls = list({
                ...
                and _is_allowed_tempo_url(href.split("#")[0].strip())
            })
...
if __name__ == "__main__":
    ...
    result = main_tempo(keyword="bola", tanggal="2026-04-28")

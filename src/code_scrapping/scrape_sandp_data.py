...
#                     print(f"  referenceData: {reference_data}")
#                     print(f"  uom: '{uom}', currency: '{currency}', description: '{description}'")
...
# def scrape_spglobal_historical(symbols=None, start_date="2023-01-01", end_date="2026-01-01",
#                                 fields=None, output_filename="spglobal_historical"):
#     access_token, refresh_token = load_tokens()
#     if not access_token or not refresh_token:
#         print("Gagal memuat token dari file")
#         return None
#     df = get_historical_data(access_token, symbols, start_date, end_date, fields)
#     if df is None:
#         print("Mencoba refresh token...")
#         new_access_token, _ = refresh_access_token(refresh_token)
#         if new_access_token:
#             df = get_historical_data(new_access_token, symbols, start_date, end_date, fields)
#         else:
...
def get_current_data(access_token, symbols, fields=None):
    ...
    try:
                ...
                if not data_list:
                    ...
                    for data_point in data_list:
                        ...
                        if mod_date and 'T' in mod_date:
                            mod_date_only = mod_date.split('T')[0]
                        else:
                            mod_date_only = mod_date
                        flat_data.append({
                            'symbol': symbol,
                            'uom': uom,
                            'currency': currency,
        ...
        if 'modDate_date' in df.columns:
            df = df.drop(columns=['modDate_date'])
        print("\nSample data:")
        print(df.head(3))
        return df
    except requests.exceptions.RequestException as e:
...
if __name__ == "__main__":
    ...
    fields = ["UOM", "Currency", "description"]
    print("\n" + "=" * 60)
    print("S&P GLOBAL SAF DATA SCRAPER")

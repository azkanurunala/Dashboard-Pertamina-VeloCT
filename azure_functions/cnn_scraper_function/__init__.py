"""
CNN News Scraper Azure Function.
HTTP-triggered function for scraping CNN news articles with keyword filtering and date range support.
"""
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    """
    Main Azure Function entry point for CNN news scraping.
    ...
    logger.info('CNN scraper function triggered')
    
    try:
        # Parse request parameters
        params = _parse_request_parameters(req)
        logger.info(f"Scraping CNN with parameters: {params}")
        
        # Run the scraping operation
        result = asyncio.run(_scrape_cnn_news(params))
        ...
        logger.error(f"CNN scraper function error: {str(e)}", exc_info=True)
        return func.HttpResponse(
            json.dumps({
                "status": "error",
...
async def _scrape_cnn_news(params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Perform CNN news scraping operation.
    ...
    try:
        ...
        async with CNNNewsScraper() as scraper:
            ...
            logger.info(f"Successfully scraped {len(articles)} articles from CNN")
        ...
        execution_time = (datetime.utcnow() - start_time).total_seconds()
        logger.error(f"CNN scraping failed after {execution_time}s: {e}")
        raise

"""
Kontan BBM Specialized Scraper for Azure Functions.
Scrapes fuel (BBM) related news from Kontan website.
"""
...
class KontanBBMScraper(BaseNewsScraper):
    ...
    def __init__(self, config: Optional[ScrapingConfig] = None):
        """Initialize Kontan BBM scraper."""
        if config is None:
            config = ScrapingConfig(
                source_name="Kontan BBM",
    ...
    def _parse_kontan_date(self, date_str: str) -> Optional[str]:
        ...
        # Handle various formats: "Senin, 15 Januari 2024 | 10:30 WIB"
        bulan = {
    ...
    async def _scrape_articles_from_source(
        ...
        **kwargs
        ...
        """Scrape BBM-related articles from Kontan."""
        try:
            # Add BBM to search query
            query = keywords[0] if keywords else "BBM"
            target_date = end_date.strftime("%Y-%m-%d")
            max_pages = kwargs.get('max_pages', 3)
            
            self.logger.info(f"Scraping Kontan BBM for: {query}, date: {target_date}")
            ...
            self.logger.info(f"Successfully scraped {len(articles)} BBM articles from Kontan")
            return articles
            
        except Exception as e:
            raise ScrapingError(f"Failed to scrape Kontan BBM: {str(e)}", source=self.source_name)


# Azure Function wrapper
async def scrape_kontan_bbm_news(
    keywords: List[str], 
    start_date: datetime, 
    end_date: datetime, 
    **kwargs
) -> List[NewsArticle]:
    """Azure Function entry point for Kontan BBM news scraping."""
    async with KontanBBMScraper() as scraper:

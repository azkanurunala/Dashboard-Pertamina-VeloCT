...
# Allowed subdomains for section filtering
BISNIS_ALLOWED_SUBDOMAINS: list[str] = [
    "market.bisnis.com",
...
def parse_article_card(item, target_date: str, target_dt: datetime) -> tuple[dict | None, bool]:
    ...
    try:
        ...
        if not any(subdomain in link for subdomain in BISNIS_ALLOWED_SUBDOMAINS):
            print(f"[Filter] Skipped (subdomain not allowed): {link[:80]}")
            return None, False

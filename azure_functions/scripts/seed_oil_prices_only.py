...
async def seed():
    ...
    target_file = r'azure_functions\references\data\(Data)Harga Minyak.csv'
    ...
    try:
        ...
        data = seeder.map_oil_prices(df)
        logger.info(f"Mapped {len(data)} rows")
        
        # Clear existing data
        logger.info("Truncating table data_oil_prices...")
        await db_handler.execute_query("TRUNCATE TABLE data_oil_prices")
        ...
        for i in range(0, len(data), chunk_size):
            ...
            try:
                await db_handler.save_structured_data('data_oil_prices', chunk)
        ...
        logger.info("Successfully seeded data_oil_prices")
